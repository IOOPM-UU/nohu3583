...
bool add_merch(db_t *db, char *name, char *desc, int price) {
    if (ioopm_hash_table_has_key(db->merch_ht, ptr_elem(name))) {
        ...
        return false; // false means did not add, item already exists
    }
    ...
    ioopm_hash_table_insert(db->merch_ht, ptr_elem(merch->name), ptr_elem(merch));
    return true;
}


// 2.1.2 List Merchandise
// This should list all items in the store. Items should preferably (soft requirement) be printed in alphabetical order on their names.
// Because there may be more things in the database than might fit on a screen, items should be printed 20 at a time, and the user is asked to continue listing (if possible) or return to the main menu.
...
void print_merchandise(db_t *db) {
    ...
    while (count < size) {
        ...
        if (count < size) {
            printf("Continue listing? (N/n to stop): ");  //To-do add ask_question
            char input[10];
...
bool change_merch(db_t *db, char *old_name, char *new_name, int new_price, char *new_desc, char *conf_string ) {
    if(conf_string[0] == 'Y' || conf_string[0] == 'y'){
    option_t res = ioopm_hash_table_lookup(db->merch_ht, ptr_elem(old_name));
    if (!res.success) {
        printf("Merchandise does not exist");
        return false;
    }
    option_t no_dupe = ioopm_hash_table_lookup(db->merch_ht, ptr_elem(new_name));
...
static stock_t *create_stock(char *shelf, int no_items) {
...
// helper function that checks if cart_id exists in list of carts.
...
// The action code shall be "R". The action shall read the id of the cart and a confirmation string. 
// The cart shall be removed only if the confirmation string begins with "Y" or "y".
...
// All possible orders in the system must be fulfillable. For example, we may only have one or more carts with 12 items of a merch X if the total stock of X in the system at least 12.
...
bool add_to_cart(db_t *db, merch_t *merch, int amnt, cart_t *cart) {
    ...
    if (!res.success) {
        if (merch->total_stock < amnt) {
            ...
            ioopm_hash_table_insert(cart->items, ptr_elem(merch->name), int_elem(amnt));
            return true;
        }
    } else {
...
bool remove_from_cart(db_t *db, merch_t *merch, int amnt, int cart_id) {
    // first check if the cart even exists
    int index = cart_index(db, cart_id);
...
// Calculate the cost of a shopping cart. If a cart holds 2 items of a merch M1 with a price of 50 and 8 items of a merch M2 with a price of 3 
// the cost of the cart is 2×50+8×3=124
